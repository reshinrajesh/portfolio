...
export default function ThirdPartyStatus() {
    ...
    return (
        <div className="p-6 rounded-3xl border border-console-fg/5 bg-console-panel/40 backdrop-blur-md">
            <h3 className="text-sm font-bold text-console-dim uppercase tracking-widest mb-4">Upstream Providers</h3>
            <div className="space-y-3">
                {SERVICES.map(service => (
                    <div key={service.name} className="flex items-center justify-between p-3 rounded-xl bg-console-fg/5 hover:bg-console-fg/10 transition-colors">
                        ...
                        <div className="flex items-center gap-2 text-xs font-bold">
                            {!statuses[service.name] ? (
                                <span className="text-console-dim flex items-center gap-1"><Loader2 size={12} className="animate-spin" /> Checking...</span>
                            ) : statuses[service.name] === "All Systems Operational" || statuses[service.name] === "Operational" ? (
                                <span className="text-console-ok flex items-center gap-1"><CheckCircle size={12} /> Operational</span>
                            ) : statuses[service.name] === "Outage" || statuses[service.name] === "Major Outage" ? (
                                <span className="text-console-down flex items-center gap-1"><XCircle size={12} /> Outage</span>
                            ) : statuses[service.name] === "Unknown" ? (
                                <span className="text-console-dim">Unknown</span>
                            ) : (
                                <span className="text-console-warn flex items-center gap-1"><XCircle size={12} /> Issues</span>
                            )}
                        </div>
                    </div>
                ))}
